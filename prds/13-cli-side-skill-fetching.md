...
**Status**: In Progress — **M0–M3 complete and verified.** M0 (companion `vfarcic/dot-ai` ingestion endpoint, [PR #655](https://github.com/vfarcic/dot-ai/pull/655)); M1 (flags + validation, `cf80617`); M2 (`--repo-dir` upload/render via `?source=`, verified against the list-by-source mock `sha256:d3dec73166…`, `760567f`); M3 (`--repo-fetch` host-git clone + upload, `02d90cb`) — argv-safe clone, `.git` stripped, `GIT_ALLOW_PROTOCOL` pinned, credentials scrubbed on every sink, symlink-escape contained; reviewed + security-audited (incl. re-audit), zero blocking. **Next: M4** (clone cache: XDG dir, shallow+incremental fetch, per-URL `flock`, `--no-cache`, content-hash upload gating, `skills cache prune`). M4 carry-over from M3: set `cmd.WaitDelay`/process-group kill so an orphaned grandchild can't outlive the clone timeout. Scope below is deliberately re-cut for the **post-#16 reality** (see next section).
