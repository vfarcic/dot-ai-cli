...
**Status**: In Progress — **M0–M4 complete and verified.** M0 (companion `vfarcic/dot-ai` ingestion endpoint, [PR #655](https://github.com/vfarcic/dot-ai/pull/655)); M1 (flags + validation, `cf80617`); M2 (`--repo-dir` upload/render via `?source=`, verified against the list-by-source mock `sha256:d3dec73166…`, `760567f`); M3 (`--repo-fetch` host-git clone + upload, `02d90cb`); M4 (clone cache + concurrency + `--no-cache` in `e8bb550`, then content-hash upload gating + evicted-source re-upload/retry + `skills cache prune` in `3d2a7a1`) — incl. the M3 `WaitDelay`/process-group-kill carry-over. All reviewed + security-audited (credential-at-rest persistence empirically verified clean, prune `RemoveAll` symlink-safe), zero blocking. **Next: M5** (composition across env-var + `--repo` + `--repo-fetch` + `--repo-dir` with first-source-wins; `--install-hook` round-trip of the new flags), then **M7** (docs). Scope below is deliberately re-cut for the **post-#16 reality** (see next section).
