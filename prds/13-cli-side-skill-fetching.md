...
**Status**: In Progress — **M0–M2 complete and verified.** M0 (companion `vfarcic/dot-ai` ingestion endpoint, [PR #655](https://github.com/vfarcic/dot-ai/pull/655)); M1 (flags + validation, `cf80617`); M2 (`--repo-dir` local source upload/render via `?source=`, `86ef73a`) — the **list-by-source endpoint** (`GET /api/v1/prompts?source=`) was added by PRD #647 and the mock re-pinned to `sha256:d3dec73166…`, so the primary "generate brand-new skills from an uploaded source" path is now verified end-to-end (`760567f`). **Next: M3** (`--repo-fetch` network source — reuses the M2 upload/list/render chain, swapping local-dir read for a host-git clone). Scope below is deliberately re-cut for the **post-#16 reality** (see next section).
