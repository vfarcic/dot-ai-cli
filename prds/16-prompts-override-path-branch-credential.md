...
- [ ] **C1 — Flags + request wiring.** Add `--repo-path` / `--repo-branch`; thread them onto the override query params for `GET /api/v1/prompts` and `POST /api/v1/prompts/:name`. Usage error when supplied without `--repo`.
- [ ] **C2 — Token forwarding.** Read `DOT_AI_GIT_TOKEN` from the CLI host env; when set and `--repo` is in use, send it as the `X-Dot-AI-Git-Token` header on override requests only. Confirm it is never logged or written to skill frontmatter.
- [ ] **C3 — `prompts refresh` override (if exposed).** Same `repo`/`path`/`branch` as JSON body fields + token header.
- [ ] **C4 — Tests against the pinned mock.** See Verification. ✅ *Prerequisite done:* mock pinned by digest `sha256:a32e2b75c7f1178723ef21611c7889ecd12e38c9d460154890d525956c7793d4` in `docker-compose.yml` (contract verified). Remaining: write the C4 assertions below.
- [ ] **C5 — Docs.** CLI flag reference, the `DOT_AI_GIT_TOKEN`-forwarding behavior, and a multi-source example (org-wide public source + a private cross-realm source under `skills/` on a non-default branch).
...
- [ ] **Param mapping on every request.** With `--repo`, `--repo-path`, and `--repo-branch` set, assert the mock receives `?repo=&path=&branch=` (query) on the list call **and** on each get-by-name call, and `repo`/`path`/`branch` in the body on refresh.
- [ ] **Path + branch actually resolve.** The mock exposes a marker prompt named **`skill-on-branch`** that is returned **only** when a repo override carries **both** a valid `path` and a valid `branch`. Assert `skills generate` surfaces `skill-on-branch` when both flags are passed, and does **not** when either is missing. (The mock's `refresh` override fixture reports `promptsLoaded: 5` for this distinct set.)
- [ ] **Token transport.** Assert the mock receives the credential as the `X-Dot-AI-Git-Token` header and **never** as a query param or body field. Assert the token is accepted but never echoed back in `source` or any response field.
- [ ] **Token gating.** With `DOT_AI_GIT_TOKEN` unset, assert no `X-Dot-AI-Git-Token` header is sent. With it set but no `--repo`, assert the header is not sent on the (non-override) requests.
- [ ] **`source` stability.** Assert `source` for a given repo is identical across requests with and without `path`/`branch`/token, and that skill tagging keys off it unchanged.
- [ ] **Error scoping.** Assert an invalid `--repo-path` (e.g. `../etc`) or `--repo-branch` (illegal characters) yields a request-scoped `400` (`VALIDATION_ERROR`) surfaced as a per-source CLI error, with no token/credentialed URL in output.
- [ ] **No secret leakage.** Grep generated skill files and captured CLI logs for the token and for any embedded-credential URL — both must be absent (credentialed `repo` URLs appear only as the scrubbed `source`).
...
- **Token forwarding default.** Forward `DOT_AI_GIT_TOKEN` automatically on every `--repo` request when the env var is set (the lean taken above, matching the per-hook model from `#575`), or gate it behind an explicit opt-in flag? Default: forward when set; revisit if an explicit gate is wanted for hooks that should not leak the token to a given server.
- **`prompts refresh` flag surface.** Whether to expose `--repo-path` / `--repo-branch` on `dot-ai prompts refresh`, or keep override-refresh body-only / scripted. Decide in C3.
