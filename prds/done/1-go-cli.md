...
- [x] **M9+M10+M16: CI/CD release pipeline + Homebrew + Scoop** — GitHub Actions workflow triggered by `repository_dispatch` from the `dot-ai` repo on each server release. Fetches `schema/openapi.json`, builds multi-arch binaries, publishes GitHub Release with the same version tag as the server. Includes Homebrew tap and Scoop bucket (Windows) with automated formula/manifest updates. Feature request filed with dot-ai for `repository_dispatch` trigger via `/request-dot-ai-feature`. Tested end-to-end with v1.1.0 release
- [x] **M11: Documentation** — Setup and installation docs in this repo, following the same pattern as the stack and Web UI repos (each component repo owns its setup docs for the devopstoolkit.ai docs site). Covers: installation (binary download, Homebrew, Scoop), configuration (server URL, token, output format), shell completion setup, skills generation, verification, and basic usage examples. Feature guides (query, recommend, remediate, etc.) belong on the docs site, not here — they describe server capabilities, not CLI-specific behavior. Evidence: Created docs/ structure with index.md, quick-start.md (agent setup), setup/ guides (installation, configuration, shell-completion), and guides/ (cli-commands-overview, skills-generation, output-formats, automation). Added docs workflow to trigger website. CLI docs published at https://devopstoolkit.ai/docs/cli/. Stack quickstart updated to present MCP and CLI options via feature requests.
- [x] **M14: Request docs site restructuring** — Create a GitHub issue on `dot-ai` requesting a documentation restructuring. The current docs at devopstoolkit.ai live entirely under `/docs/mcp/` and conflate server capabilities with the MCP access method. The issue should propose splitting into: (1) server capabilities (feature guides, tools overview — access-method-agnostic, examples can use MCP but should note CLI and Web UI parity), and (2) setup/access methods (MCP config, CLI install, Web UI setup — each as a peer). The CLI has full feature parity with MCP — from the agent's perspective, multi-step workflows are orchestrated identically regardless of transport. The issue provides context for the dot-ai agent to create a PRD for the restructuring. Evidence: Feature request filed via `/request-dot-ai-feature`. dot-ai created PRD #373 to restructure docs into "AI Engine" (capabilities) and "MCP" (access method) top-level sections. Will receive path mapping table once their M1-M5 complete for updating CLI docs cross-references.
