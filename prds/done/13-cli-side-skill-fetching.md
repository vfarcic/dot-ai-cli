...
**Status**: Complete — all milestones M0–M7 implemented, reviewed, and security-audited (incl. CodeRabbit on PR #18); full integration suite green. Merging via CLI **PR #18** as the joint release with the companion `vfarcic/dot-ai` **PR #655** (already merged). Milestone history below.

<!-- prior status detail retained for history -->
**M0–M4 complete and verified.** M0 (companion `vfarcic/dot-ai` ingestion endpoint, [PR #655](https://github.com/vfarcic/dot-ai/pull/655)); M1 (flags + validation, `cf80617`); M2 (`--repo-dir` upload/render via `?source=`, verified against the list-by-source mock `sha256:d3dec73166…`, `760567f`); M3 (`--repo-fetch` host-git clone + upload, `02d90cb`); M4 (clone cache + concurrency + `--no-cache` in `e8bb550`, then content-hash upload gating + evicted-source re-upload/retry + `skills cache prune` in `3d2a7a1`) — incl. the M3 `WaitDelay`/process-group-kill carry-over; M5 (composition across all four sources + `--install-hook` round-trip of the new flags in `8452930`, incl. a `shellQuote` fix closing a hook-command shell-injection). All reviewed + security-audited (credential-at-rest persistence empirically verified clean, prune `RemoveAll` symlink-safe, hook RCE fully closed), zero blocking. M6 (consolidated Success-Criteria integration coverage — all 8 criteria mapped to e2e tests; the one gap, `--repo-fetch` argument-substitution render + a `--repo-fetch` `?source=`-not-`?repo=` wire test, filled in `9c1f1a0`). **Only M7 remains: docs** (flag reference, the cache + `cache prune`, the `--repo-dir` opt-in/security model, a worked multi-source composition example, and explicit guidance to prefer #16 `--repo` + `DOT_AI_GIT_TOKEN` when a static credential works). Scope below is deliberately re-cut for the **post-#16 reality** (see next section).
...
- [x] **M2 — `--repo-dir` local source end-to-end.** Read source from a local dir, upload to the ingestion endpoint, tag `source: local:<label>`, render an argument-taking skill correctly via the server. Security posture: `DOT_AI_ALLOW_REPO_DIR=1` opt-in + path refusals.
- [x] **M3 — `--repo-fetch` network source end-to-end.** CLI clone via the host git stack, upload source, tag `source: <scrubbed-url>`, render correctly. Verify a source the server cannot reach but the host can.
- [x] **M4 — Clone cache, concurrency, GC.** XDG cache dir, shallow clone + incremental fetch, per-URL `flock`, `--no-cache`, content-hash upload gating, `skills cache prune --older-than`.
- [x] **M5 — Composition + hook round-trip.** Source-tagged wipe-own-slice and first-arrived-wins verified across env-var + `--repo` + `--repo-fetch` + `--repo-dir`; `--install-hook` emits and round-trips the new flags.
- [x] **M6 — Tests against the updated mock.** Integration/e2e coverage for all of the above against the mock once it exposes the ingestion endpoint (per the project's integration-test convention).
- [x] **M7 — Docs.** Flag reference, the cache + `cache prune`, the `--repo-dir` opt-in/security model, a worked multi-source composition example, and explicit guidance to **prefer #16 (`--repo` + `DOT_AI_GIT_TOKEN`) whenever a static credential works**.
