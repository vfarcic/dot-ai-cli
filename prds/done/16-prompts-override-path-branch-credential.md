...
- ~~**LOW** — The server-message credential-scrub regex (`RedactCredentials`) matches `user:pass@` but misses username-only PAT URLs (`https://TOKEN@host`).~~ **Resolved** (CodeRabbit follow-up on PR #17): the regex now uses an optional password group (`://[^/@\s]+(?::[^/@\s]*)?@`), covering username-only PAT URLs and encoded-colon credentials; idempotent and non-over-redacting (table-driven unit test `TestRedactCredentials`).
